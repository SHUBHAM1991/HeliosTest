...
$(document).ready(function() {
 ...
 $("body").click(function(){ 
	...
	if((undefined !=status) && (status=='block') && (sidebarClick == true)){
 	...
 	//populateCustomerProfile(); // this call contains showStoresNearBy call with zipcode
...
function populateCustomerProfile(){
	...
	$.ajax({
		...
		success : function(data, textStatus, jqXHR) {
		 	if(data !=null && data !='null' && null !=data.customerInfo && 'null' !=data.customerInfo){
				$.each(data.customerInfo, function(i, item) {
					...
					if(custid == tmpCustNumber){
				        ...
				        customerZipCode = checkUundefined(item.postalCode); // For showStoresNearBy method zipcode is input, while loading page
						...
						if (checkUundefined(item.phone).length > 0)
							$("#custConNum").html(' '+'<span onclick="javascript:logUserActivityDotCom(8090,\''+checkUundefined(item.phone)+': clicked in SAM Store Locator Page for Customer '+custid+'\')">'+checkUundefined(item.phone)+'</span>');
				...
				$('#childInfoId').popover({
			        ...
			        logUserActivityDotCom(7039, 'viewed sub rewards from SAM store nearby');
		 	...
		 	showStoresNearBy({zipCode : customerZipCode});
...
function searchCustomer(searchText,acctId){
	...
	$.ajax({
		...
		success : function(data, textStatus, jqXHR) {
			...
			logUserActivityDotCom(7045, 'User has perform global search from SAM stores nearby');
        },
									...
									dotcomActivityVOArr[0]=data.staplesDotcomActivityVo;
										...
										mkttrHTML += '<td class="datatablesTd">'
											+checkUundefined(data.contactFullName) +'</td>'
											 ...
											 +'<td class="datatablesTd">'+'<span onclick="javascript:logChangeSatusActivity(8090,\''+checkUundefined(data.phoneNum)+': clicked from Users Grid in SAM Dashboard Page for Customer '+custid+'\')">'
...
function onFindStoresNearByFilterClick() {
	...
	logUserActivityDotCom(7046, 'User has applied advanced search to find stores nearby');
}

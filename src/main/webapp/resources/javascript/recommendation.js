	...
	$.ajax({
		...
		success : function(data, textStatus, jqXHR) {
	      ...
	      //LTS saving changes
	        ...
	        //alert("-->"+data.listOfCustProfileVO[0].companyAddr1+"----"+alert("-->"+data.listOfCustProfileVO[0].companyAddr2));
	        ...
	        if((data.listOfCustProfileVO[0].companyPhone != undefined && data.listOfCustProfileVO[0].companyPhone != '') && (address !='')){
	        	address=address+'<span onclick="javascript:logChangeSatusActivity(8088,\''+formatPhone(data.listOfCustProfileVO[0].companyPhone)+': clicked in Recommendation Grid\','+custid+')">'+formatPhone(data.listOfCustProfileVO[0].companyPhone)+'</span>';
	        	...
	        	address="P. "+'<span onclick="javascript:logChangeSatusActivity(8088,\''+formatPhone(data.listOfCustProfileVO[0].companyPhone)+': clicked in Recommendation Grid\','+custid+')">'+formatPhone(data.listOfCustProfileVO[0].companyPhone)+'</span>';
	        }else if((address !='') && (data.listOfCustProfileVO[0].companyPhone == undefined || data.listOfCustProfileVO[0].companyPhone == '')){
...
function getBoughtAlsoBught(orderContact) {
	if (undefined != orderContact && orderContact != 'select') {
		...
		$
				.ajax({
					url : ctx + "/getBoughtRecommendation/" + custid +"/"+accId,
					type : "POST",
					cache : false,
					data : formData,
					timeout : 1000000,
...
function getReorderRecomm() {
	...
	$
			.ajax({
				url : ctx + "/getReorderRecommendations/customer/" + custid +"/user/"+accId ,
								...
								if(data.objPurchaseDetailsListVO !=null && data.objPurchaseDetailsListVO !=undefined){
									$.each(data.objPurchaseDetailsListVO, function(i, item) {
										...
										if (item.savingCategory != undefined &&
											...
											mkttrHTML += '<i id="OrderIdDtl'+cnt+'"  class="fa fa-info-circle fa-lg" style=" font-size: 120% !important; color: #0066c0;" tabindex="0" class="" role="button" data-toggle="popover" data-trigger="hover"></i><div style="" id="orderdivId'+cnt+'" class="toolTip"><div style="font-size:12px;padding-top:5px;padding-left:10px;padding-right:10px;color:#0066c0;letter-spacing:1px;"><u><b>Premium Savings Info</b></u> : </div>'
											 $.each(item.savingCategory, function(k, category){
												 ...
												 if((checkUundefined(category))=='LOR')
												      category	='Large Order Rebate'; 
												 mkttrHTML +='<div style="font-size:12px;padding-top:5px;padding-left:10px;padding-right:10px;color:crimson;letter-spacing:1px;">'+toCamCase(removeUnderScore(checkUundefined(category)))+': $'+(checkUundefined(Amount[k])) +'</div>'
										...
										mkttrHTML += '</td>'
											 ...
											 +'<td class="datatablesT formatClsOrd">'
											 + chkNegAmount(formatNum(checkUundefined(item.netSpendAmount))) +'</td>'
											 +'<td class="datatablesTd">'
											 + checkUundefined(item.orderContact) +'</td>'
											 +'<td class="datatablesTd">'
							              ...
							              if (hasContactDetails) {
							            	  ...
							            	  contactDetails = '<div style="" id="orderContact' + cnt +'" class="toolTip">'+
																...
																+'<a href="javascript:showSfdcPopUp(\''+userSfdcUrl+'\');"><i class="fa fa-plus-square" aria-hidden="true" style="padding-left: 5px;font-size: 116%;vertical-align: middle;"></i></a></div>'
							              						...
							              						'<div style="padding-left:5px;letter-spacing:1px;">Phone: <span style="font-size:12px;padding-top:5px;padding-left:10px;padding-right:10px;color:deepskyblue;" onclick="javascript:logChangeSatusActivity(8088,\''+contactPhone+': clicked in Recommendation User Details Grid\','+custid+')">'+ contactPhone + '</span></div>'+
							              						'</div>';
							              } else {
						...
						if(data.notiInfoVOList !=null && data.notiInfoVOList !=undefined && data.notiInfoVOList !=''){ 
							$.each(data.notiInfoVOList, function(i, item) {
								if(item.sementType=='Growth')
									 GHTML +='<li class="list-group-item" style="color:#bbb;border-bottom: 2px solid #999 !important;border-bottom-style: dotted !important;height: auto!important;padding: 13px 0px 15px 0px !important;font-family:Helvetica;font-size: 13px;"><a href="javascript:openSubCallToAction(\''+item.segmentDesc+'\');" style="">'+item.segmentDesc+'</a><span class="pull-right text-muted small"></span></li>'
									else if(item.sementType=='Retention')
									 RHTML +='<li class="list-group-item" style="color:#bbb;border-bottom: 2px solid #999 !important;border-bottom-style: dotted !important;height: auto!important;padding: 13px 0px 15px 0px !important;font-family:Helvetica;font-size: 13px;"><a href="javascript:openSubCallToAction(\''+item.segmentDesc+'\');" style="">'+item.segmentDesc+'</a><span class="pull-right text-muted small"></span></li>'
									else if(item.sementType=='Expansion')	
									 EHTML +='<li class="list-group-item" style="color:#bbb;border-bottom: 2px solid #999 !important;border-bottom-style: dotted !important;height: auto!important;padding: 13px 0px 15px 0px !important;font-family:Helvetica;font-size: 13px;"><a href="javascript:openSubCallToAction(\''+item.segmentDesc+'\');" style="">'+item.segmentDesc+'</a><span class="pull-right text-muted small"></span></li>'
